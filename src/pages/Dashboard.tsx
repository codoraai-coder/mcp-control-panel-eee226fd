import { FileText, Workflow, Calendar, Loader2, CheckCircle, XCircle, Clock, RefreshCw } from "lucide-react";
...
export default function Dashboard() {
  const { stats, recentActivity, jobs, isLoading, refetch } = useDashboardData();
        ...
        {isLoading ? (
          ...
          <>
            ...
            <StatCard
              ...
              icon={Loader2}
              description={stats.runningJobs > 0 ? "MCP is working" : "No active jobs"}
              variant="amber"
            />
          </>
        )}
          ...
          <CardContent className="space-y-4">
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-4">
                No active jobs. Start generating content to see progress here.
              </p>
            ) : (
              jobs.map((job) => (
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-medium text-sm truncate max-w-[200px]">{job.workflowName}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    {job.status === 'running' && (
        ...
        <Card className="border-border/50 shadow-sm">
          ...
          <CardContent className="space-y-4">
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-4">
                No recent activity. Generate some content to get started.
              </p>
            ) : (
